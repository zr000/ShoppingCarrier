...
void CSubclassWndMap::Add(HWND hwnd, CSubclassWnd* pSubclassWnd)
{
	...
	pSubclassWnd->m_pNext = Lookup(hwnd); //把map中当前的subclasswnd对象放到新对象的pNext
	SetAt(hwnd, pSubclassWnd); //新对象插入map
	
	if (pSubclassWnd->m_pNext==NULL)   //如果是新插入的
    {
		// If this is the first hook added, subclass the window
		pSubclassWnd->m_pOldWndProc = //保存老的winproc函数
    ...
    else //如果不是新插入的
    {
		...
		pSubclassWnd->m_pOldWndProc = pSubclassWnd->m_pNext->m_pOldWndProc;  //复制老的winproc函数
	}
...
void CSubclassWndMap::Remove(CSubclassWnd* pUnHook)
{
	...
	ASSERT(pHook);
	if (pHook==pUnHook)  //当前的CSubclassWnd
    {
		// hook to remove is the one in the hash table: replace w/next
		if (pHook->m_pNext)   //有一个CSubclassWnd链
			SetAt(hwnd, pHook->m_pNext);
		else   //仅有一个CSubclassWnd
        {
			// This is the last hook for this window: restore wnd proc
			RemoveKey(hwnd);   //删掉键
			SetWindowLong(hwnd, GWL_WNDPROC, (DWORD)pHook->m_pOldWndProc);    //恢复原来的winproc
		}
	} 
    else  //不是当前的CSubclassWnd
    {
		// 从链表中删除CSubclassWnd
...
//
void CSubclassWndMap::RemoveAll(HWND hwnd)    //释放该hwnd的CSubclassWnd
{
